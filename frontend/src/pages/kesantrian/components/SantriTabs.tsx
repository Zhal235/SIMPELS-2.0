interface SantriTabsProps {
  tabs: string[]
  active: string
  onChange: (tab: string) => void
}

export default function SantriTabs({ tabs, active, onChange }: SantriTabsProps) {
  return (
    <div className="relative z-10 flex flex-wrap gap-2 border-b border-gray-200 bg-gray-100 rounded-t-lg p-1 shadow-sm mb-4">
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            onClick={(e) => { e.preventDefault(); onChange(tab) }}
          >
            {tab}
          </button>
        )
