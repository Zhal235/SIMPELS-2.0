...
class AsramaController extends Controller
{
    ...
    public function tambahAnggota(Request $request, string $id)
    {
        ...
        $validated = Validator::make($request->all(), [
            'santri_id' => 'required|string|exists:santri,id',
        ])->validate();
        ...
        if (!empty($santri->asrama_id) && (int)$santri->asrama_id !== (int)$asrama->id) {
            return response()->json(['status' => 'error', 'message' => 'Santri sudah terdaftar di asrama lain.'], 422);
        }
        $santri->asrama_id = $asrama->id;
        $santri->asrama_nama = $asrama->nama_asrama;
        $santri->save();
        return response()->json(['status' => 'success', 'message' => 'Anggota berhasil ditambahkan', 'data' => $santri]);
    }
