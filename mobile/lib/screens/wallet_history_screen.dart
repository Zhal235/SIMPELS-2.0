...
class WalletHistoryScreen extends StatefulWidget {
  ...
  const WalletHistoryScreen({super.key, required this.santriId, required this.santriName});
  ...
  Future<void> _loadWallet() async {
    ...
    try {
      ...
      if (res.statusCode == 200 && res.data['success'] == true) {
        setState(() => _walletData = Map<String, dynamic>.from(res.data['data'] ?? {}));
      }
    } catch (e) {
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                onSaved: (v) => amount = double.tryParse(v!.replaceAll('.', '')),
              ),
            ],
          ),
        ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              Navigator.pop(context);
              try {
                final res = await _api.setSantriDailyLimit(widget.santriId, newVal!);
                if (res.statusCode == 200 && res.data['success'] == true) {
                  if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Limit diperbarui')));
                  await _loadWallet();
                  await Provider.of<AuthProvider>(context, listen: false).refreshData();
                } else {
                  if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal memperbarui limit: ${res.data}')));
                }
              } catch (e) {
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      const Text('Saldo Dompet', style: TextStyle(color: Colors.grey)),
                                      const SizedBox(height: 8),
                                      Text('Rp ${_formatCurrency(_walletData?['saldo'] ?? 0)}', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
                                    ],
                                  ),
                                  Column(
                                    crossAxisAlignment: CrossAxisAlignment.end,
                                    children: [
                                      Text('Limit Harian: Rp ${_formatCurrency(_walletData?['limit_harian'] ?? 0)}', style: const TextStyle(color: Colors.grey)),
                                      TextButton.icon(onPressed: _showEditLimitDialog, icon: const Icon(Icons.edit), label: const Text('Ubah')),
                                    ],
                                  ),
                                ],
                              ),
                              ...
                              Row(
                                ...
                                children: [
                                  ElevatedButton.icon(onPressed: _showTopupDialog, icon: const Icon(Icons.add), label: const Text('Top-up')),
                      ...
                      if ((_walletData?['transaksi_terakhir'] ?? []).isEmpty)
                        const Center(child: Padding(padding: EdgeInsets.symmetric(vertical: 32), child: Text('Belum ada transaksi')))
                      else
                        ...(_walletData?['transaksi_terakhir'] ?? []).map<Widget>((t) {
                          final txn = WalletTransaction.fromJson(Map<String, dynamic>.from(t));
                          final isCredit = txn.tipe == 'credit';
                          // Format tanggal: 30/11/2025, 11:41
                          final formattedDate = '${txn.tanggal.day.toString().padLeft(2, '0')}/${txn.tanggal.month.toString().padLeft(2, '0')}/${txn.tanggal.year}, ${txn.tanggal.hour.toString().padLeft(2, '0')}:${txn.tanggal.minute.toString().padLeft(2, '0')}';
                          
                          return Card(
                            margin: const EdgeInsets.symmetric(vertical: 6),
                            child: ListTile(
                              title: Text(txn.keterangan.isNotEmpty ? txn.keterangan : (isCredit ? 'Top-up' : 'Pembayaran')),
                              subtitle: Text(formattedDate),
                              trailing: Text('${isCredit ? '+' : '-'} Rp ${_formatCurrency(txn.jumlah)}', style: TextStyle(color: isCredit ? Colors.green : Colors.red, fontWeight: FontWeight.bold)),
                            ),
                          );
                        }).toList(),
                    ],
